package interfaces;

import java.rmi.Remote;
import java.rmi.RemoteException;
import java.util.HashMap;

/**
 * Interface Paddock
 */

public interface PaddockInterface extends Remote {
    /**
     * Go check the horses
     * @param specId Spectator ID
     * @return winningChance - Winning chance of the Horse that the spectator is betting on
     *
     *
     */
    int goCheckHorses(int specId) throws RemoteException;

    /**
     * Proceed to Paddock
     *
     * @param horseId Horse ID
     * @param horseWinningChance Horse winning chance
     */
    void proceedToPaddock(int horseId, double horseWinningChance) throws RemoteException;

    /**
     * Get the horses winning chances
     *
     * @return winningChances - List containing the winning chances for all horses
     *
     */
    HashMap getWinningChances() throws RemoteException;

    /**
     * Método para "fechar" a classe - Apenas toma
     */
    void close() throws RemoteException ;
}
